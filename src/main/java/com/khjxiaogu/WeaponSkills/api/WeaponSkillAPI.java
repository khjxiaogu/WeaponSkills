...
public class WeaponSkillAPI {
	 ...
	 * SkillDescription's skill field is not necessary,for it only check the name
	 * and level.
	 * 
	 ...
	 * @return 技能描述对象集合，如果物品没有技能则为null Set of SkillDescription object contains the
	 *         info,null if no skill is presented on this item
	 */
	...
	public void RegisterSkillHook(SkillHook hook) {
		...
		WeaponSkills.plugin.getLogger().info("plugin "+hook.getName()+" registered for a hook.");
	}
	 ...
	 * set a cool down for a player,you should check the result every time the skill
	 * triggered.
	 ...
	 * @return 如果玩家不在冷却时间内，返回0，如果玩家正在冷却中，返回剩余时间ms return 0 if target player is not
	 *         in cool down,otherwise return time rest
	 */
